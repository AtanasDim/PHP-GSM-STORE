...
$result = $mysqli->query("SELECT * FROM mobile_p where id_gsm=".$_REQUEST['del_id']);
$row = $result->fetch_assoc();

if ($row['picture']) unlink("pictures/".$row['picture']);
$mysqli->query("delete FROM mobile_p where id_gsm=".$_REQUEST['del_id']);
echo "Данните за посочения модел са изтрити.<br>";

$mysqli->close();
?>
<script>setTimeout('self.location.href="list_models.php"',2500)</script>
</div>
